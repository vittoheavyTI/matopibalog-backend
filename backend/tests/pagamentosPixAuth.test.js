...
// ── Fixtures ────────────────────────────────────────────────────────────────
//
// S1-TEST-HARDEN-01 — as fixtures precisam ser TENANT-COERENTES. A versão anterior
// provava o comportamento certo com dados que um JOIN real nunca produziria: um
// usuário com `empresa_id='emp-autonomo'` cujo `empresas.tipo` era
// 'transportadora'. `empresas` é justamente a empresa de `empresa_id` — os dois não
// podem discordar. Um harness incoerente esconde erros de tenant em vez de expô-los.
//
// Três empresas, cada usuário na sua, cada fatura na empresa do seu cenário.
const EMPRESAS = {
  'emp-autonomo': { id: 'emp-autonomo', tipo: 'autonomo' },
  'emp-transportadora': { id: 'emp-transportadora', tipo: 'transportadora' },
  'emp-outra': { id: 'emp-outra', tipo: 'transportadora' },
};

// Faturas por id, cada uma na empresa a que pertence de fato.
const FATURAS = {
  'fat-autonomo': {
    id: 'fat-autonomo', empresa_id: 'emp-autonomo', asaas_id: 'pay_auto', tipo_pagamento: 'PIX',
  },
  'fat-transportadora': {
    id: 'fat-transportadora', empresa_id: 'emp-transportadora', asaas_id: 'pay_transp', tipo_pagamento: 'PIX',
  },
};

// Usuários por uid, no formato que o resolver consulta — `empresas` sempre é a
// empresa de `empresa_id`, como num JOIN de verdade.
const USUARIOS = {
  'u-autonomo-dono': {
    id: 'u-autonomo-dono', tipo: 'motorista', empresa_id: 'emp-autonomo',
    permission_template_id: null, empresas: EMPRESAS['emp-autonomo'],
  },
  'u-motorista-vinculado': {
    id: 'u-motorista-vinculado', tipo: 'motorista', empresa_id: 'emp-transportadora',
    permission_template_id: null, empresas: EMPRESAS['emp-transportadora'],
  },
  'u-operador': {
    id: 'u-operador', tipo: 'operador', empresa_id: 'emp-transportadora',
    permission_template_id: null, empresas: EMPRESAS['emp-transportadora'],
  },
  ...
  'u-financeiro': {
    id: 'u-financeiro', tipo: 'financeiro', empresa_id: 'emp-transportadora',
    permission_template_id: 'tpl-financeiro', empresas: EMPRESAS['emp-transportadora'],
  },
  // Administrador legítimo de OUTRA empresa: tem finance.saas.view de verdade e
  // ainda assim não pode alcançar fatura alheia.
  'u-outro-tenant': {
    id: 'u-outro-tenant', tipo: 'admin', empresa_id: 'emp-outra',
    permission_template_id: null, empresas: EMPRESAS['emp-outra'],
  },
};
...
function supabaseStub() {
  function builder(tabela) {
    ...
    const b = {
      ...
      limit: () => b,
      maybeSingle: async () => {
        if (tabela === 'faturas') return { data: faturaExiste ? (FATURAS[filtroId] || null) : null, error: null };
      ...
      single: async () => {
        if (tabela === 'faturas') {
          const f = faturaExiste ? FATURAS[filtroId] : null;
...
const MOTORISTA_VINCULADO = token({ uid: 'u-motorista-vinculado', role: 'motorista', is_super_admin: false, empresa_id: 'emp-transportadora' });
const OPERADOR = token({ uid: 'u-operador', role: 'operador', is_super_admin: false, empresa_id: 'emp-transportadora' });
const FINANCEIRO = token({ uid: 'u-financeiro', role: 'financeiro', is_super_admin: false, empresa_id: 'emp-transportadora' });
const OUTRO_TENANT = token({ uid: 'u-outro-tenant', role: 'admin', is_super_admin: false, empresa_id: 'emp-outra' });

let ctx;
// Cada cenário pede a fatura da SUA empresa (ou, no caso cross-tenant, a de outra
// de propósito) — a fixture deixou de ser um id genérico.
async function pedirPix(auth, faturaId = 'fat-autonomo') {
  chamadasProvider = 0;
  const headers = {};
  if (auth) headers.authorization = `Bearer ${auth}`;
  const r = await fetch(`http://127.0.0.1:${ctx.port}/pagamentos/faturas/${faturaId}/pix`, { headers });
...
test('S1-HIGH-03: motorista vinculado → 403 e ZERO chamada ao provedor', async () => {
  // Fatura da PRÓPRIA transportadora dele: o que falta é permissão, não tenant.
  const r = await pedirPix(MOTORISTA_VINCULADO, 'fat-transportadora');
...
test('S1-HIGH-03: operador sem autoridade financeira → 403, provedor intocado', async () => {
  const r = await pedirPix(OPERADOR, 'fat-transportadora');
  assert.equal(r.status, 403);
  assert.equal(r.chamadasProvider, 0);
});

test('S1-HIGH-03: Financeiro (tem finance.saas.view) obtém o Pix da própria empresa', async () => {
  const r = await pedirPix(FINANCEIRO, 'fat-transportadora');
  assert.equal(r.status, 200);
  assert.equal(r.chamadasProvider, 1);
});

test('S1-HIGH-03: outro tenant é negado mesmo tendo a permissão', async () => {
  // A permissão não expande fronteira de tenant: o Administrador da `emp-outra`
  // tem finance.saas.view de verdade e ainda assim não alcança a fatura da
  // `emp-transportadora`.
  const r = await pedirPix(OUTRO_TENANT, 'fat-transportadora');
  assert.equal(r.status, 403);
  assert.equal(r.chamadasProvider, 0);
});
