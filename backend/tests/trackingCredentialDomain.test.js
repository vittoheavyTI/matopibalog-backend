...
function credOk(over = {}) {
  return {
    id: 'cred-1', motorista_id: MOT, empresa_id: EMP, frete_id: FRETE, session_id: SESS, device_id: DEV,
...
const freteOk = (o = {}) => ({ id: FRETE, status: 'em_viagem', empresa_id: EMP, motorista_id: MOT, ...o });
function avaliar(over = {}) {
  return avaliarCredencial({ credencial: credOk(), usuario: usuarioOk(), sessao: sessaoOk(), frete: freteOk(), deviceId: DEV, agoraMs: AGORA, ...over });
}
...
test('tudo canônico válido → ok, identidade inclui frete_id', () => {
  const r = avaliar();
  assert.equal(r.ok, true);
  assert.equal(r.identidade.uid, MOT);
  assert.equal(r.identidade.empresa_id, EMP);
  assert.equal(r.identidade.frete_id, FRETE);
...
test('credencial ausente/revogada', () => {
  ...
  assert.equal(avaliarCredencial({ credencial: credOk({ revoked_at: new Date(AGORA).toISOString() }), usuario: usuarioOk(), sessao: sessaoOk(), frete: freteOk(), deviceId: DEV, agoraMs: AGORA }).code, 'tracking_credential_revoked');
});
...
test('teto absoluto ultrapassado → max_lifetime (nem renovação recupera)', () => {
  ...
  const base = { credencial: cred, usuario: usuarioOk(), sessao: sessaoOk(), frete: freteOk(), deviceId: DEV, agoraMs: AGORA };
...
test('expirada bloqueia telemetria mas é aceita na RENOVAÇÃO (dentro do teto)', () => {
  ...
  assert.equal(avaliarCredencial({ ...base, permitirExpirada: true }).ok, true); // renovação recupera
});
...
test('sessão ausente ou revogada → session_revoked (§5 não olha idle/absolute)', () => {
  ...
  // sessão com idle/absolute vencidos MAS não revogada → segue válida
...
test('viagem vinculada (§H-3): frete errado/ausente/de outro dono/inativo', () => {
  assert.equal(avaliar({ frete: null }).code, 'tracking_trip_mismatch');
  assert.equal(avaliar({ frete: freteOk({ id: 'outro-frete' }) }).code, 'tracking_trip_mismatch');
  assert.equal(avaliar({ frete: freteOk({ empresa_id: 'outra' }) }).code, 'tracking_tenant_mismatch');
  assert.equal(avaliar({ frete: freteOk({ motorista_id: 'outro' }) }).code, 'tracking_tenant_mismatch');
  assert.equal(avaliar({ frete: freteOk({ status: 'finalizado' }) }).code, 'tracking_trip_inactive');
  assert.equal(avaliar({ frete: freteOk({ status: 'cancelado' }) }).code, 'tracking_trip_inactive');
});
