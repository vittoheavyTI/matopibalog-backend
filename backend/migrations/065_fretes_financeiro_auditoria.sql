...
BEGIN
  ...
  INSERT INTO public.fretes_financeiro_auditoria (
    ...
    btrim(p_source),
    btrim(p_request_id),
    coalesce(nullif(btrim(p_correction_type), ''), 'manual_legacy_financial_correction'),
    v_before_snapshot,
    v_after_snapshot
  )
...
REVOKE ALL ON FUNCTION public.corrigir_frete_financeiro_legacy(uuid, uuid, uuid, text, text, text, text, jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.corrigir_frete_financeiro_legacy(uuid, uuid, uuid, text, text, text, text, jsonb)
  TO service_role;
