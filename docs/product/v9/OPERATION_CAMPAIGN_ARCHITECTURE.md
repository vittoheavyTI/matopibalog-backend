...
> `OPERATION_ORCHESTRATOR_V1_TECHNICAL_STATUS=CLOSED` (E3.1/E3.2). Auditoria confirmou que o pipeline determinístico já existia quase por completo em Campaign-A/B/C + Dispatch V1 (zero duplicação, zero fronteira errada); fechou `next_action` determinístico + fluxo guiado único de objetivo (`POST /operation-campaigns/objective`), schema-free. Detalhe: [OPERATION_ORCHESTRATOR_V1](./OPERATION_ORCHESTRATOR_V1.md).
> `CAMPAIGN_D_TECHNICAL_STATUS=CLOSED`. Fecha os 3 itens deferidos pelo Orchestrator V1: replan pós-aprovação concurrency-safe (reusa os índices únicos já existentes da migration 076, sem migration nova; concorrência provada com 2 testes reais contra Postgres 16), Route Intelligence integrada ao planejamento (bounded/deduplicado, provider continua disabled, `PRODUCTION_ROUTE_PROVIDER_CALLS=0`), multi-origem no fluxo guiado (`origins[]`, total sempre derivado). Detalhe: [CAMPAIGN_D_REPLAN_ROUTE_MULTI_ORIGIN](./CAMPAIGN_D_REPLAN_ROUTE_MULTI_ORIGIN.md).
...
- `SHIPPER_PORTAL_BOUNDARY`: **realizado** em E3.5 (PORTAL-A/PORTAL-B, fechados em producao 2026-08-25). A Campaign e originada pelo aceite da solicitacao do embarcador via handoff idempotente ao Orchestrator — o operador nao redigita nada — e o acompanhamento externo e **projecao read-only** que reusa o mapa congelado `freightExecutionStatus.js` e o servico canonico de progresso: o portal nao tem maquina de estados propria nem fabrica progresso. A proveniencia `solicitacao -> campanha -> frete -> documento/evidencia` e garantida por FKs compostas no banco (migration 081), entao o embarcador nunca ve objeto de outra campanha/empresa. Visibilidade externa de documento exige linha explicita e revogavel em `shipper_document_shares`; nada interno vaza por padrao.
...
`OPERATIONAL PROGRESS + ELIGIBILITY + DISPATCH READINESS (read-only)`.
...
- `CAMPAIGN_D_MIGRATION_REQUIRED=false` (reusa índices únicos já existentes da migration 076).
- `NEXT_STATUS=CAMPAIGN_D_CLOSED_NEXT_MACROFRONT_REQUIRES_NEW_OWNER_SCOPE` (RBV9-INV-031 CLOSED; Campaign-B/C, Dispatch V1, Operation Orchestrator V1 e Campaign-D já fechados).
