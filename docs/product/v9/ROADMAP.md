...
- **E3.5 Portal do Embarcador** (RBV9-INV-081, D-024) — 🔶 **PORTAL-A implementado, aguardando `OWNER_MIGRATION_GATE_SHIPPER_PORTAL`** (PR #475, migration `080_shipper_portal_foundation.sql`, **não aplicada**). Auditoria confirmou que não existia nenhuma entidade de embarcador (0 tabelas) e que `middlewares/tenant.js` derivaria o tenant interno inteiro se o embarcador fosse criado como `usuarios` — por isso a identidade externa vive em tabelas próprias sem `empresa_id`, e o acesso passa por relacionamento explícito e revogável. Slice A (identidade/fronteira) + Slice B (solicitação + handoff ao Orchestrator, sem redigitação) prontos; Slice C (tracking/documentos/comprovantes) e Slice D (UX do portal) ficam para PORTAL-B após o gate. Ver [SHIPPER_PORTAL_V1](./SHIPPER_PORTAL_V1.md).
