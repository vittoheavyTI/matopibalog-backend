...
| **FLEET** | Ativos, composições, documentos de ativo, odômetro, pneus, manutenção | ORG_SCOPE | **FLEET-A foundation CLOSED; Fleet-B implementada em PR** |
...
- **FLEET-B Operational Experience**: ✅ **IMPLEMENTED_IN_PR** — sem migration 075. Web `/frota` protegido por `fleet.view`, acoes por `fleet.manage`, menu gated, resumo operacional, pendencias, composicoes, ativos, pneus, manutencoes, odometro e documentos de ativo por referencia. Backend adiciona `/fleet/overview` e endpoints guiados sobre as tabelas 074. Sem backfill, sem fabricacao de legado, sem Asaas/env/storage production. Validação local: backend `1665/1665`, web `118/118`, `tsc -b && vite build`, SEC-1 local skipped pela configuracao. `OWNER_VISUAL_VALIDATION=PENDING`.
...
- **E2.2 Vínculo temporal motorista↔composição + handoff** (RBV9-INV-035/028, D-002): vínculo motorista↔ativo/composição coberto por FLEET-A e exposto em Fleet-B; handoff completo fica para slice posterior.
...
- **E2.4 Documentos de ativo + vencimentos** (RBV9-INV-036/048, D-013): fundação `asset_documents` coberta por FLEET-A; Fleet-B registra referencia/caminho e alerta vencimentos, sem Storage novo.
- **E2.5 Pneus** (RBV9-INV-038, D-005): fundação de tabelas coberta por FLEET-A; Fleet-B registra pneus e instalacao; analytics/custo por km/alertas avançados posteriores.
- **E2.6 Manutenção** (RBV9-INV-039): fundação de eventos coberta por FLEET-A; Fleet-B registra e lista preventiva/corretiva; experiencia de oficina avancada posterior.
