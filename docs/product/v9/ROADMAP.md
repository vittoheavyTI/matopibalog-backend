...
- **E3.1 Operation Orchestrator** — ✅ **CLOSED em produção**: pipeline `OBJECTIVE → NORMALIZE → VALIDATE → RESOURCE_SNAPSHOT → CAPACITY_PLAN → TRIP_PLAN → SCENARIOS → HUMAN_APPROVAL → DISPATCH_READY → FREIGHT_MATERIALIZATION → EXECUTION → VERIFY → REPLAN/EXCEPTIONS` já existia quase por completo (Campaign-A/B/C + Dispatch V1); auditoria confirmou zero duplicação e zero fronteira errada no backend. Fechou o que faltava: `next_action` determinístico (`operationOrchestratorService.js`, composição pura, sem IA) e fluxo guiado único de objetivo. Ver [OPERATION_ORCHESTRATOR_V1](./OPERATION_ORCHESTRATOR_V1.md).
- **E3.2 Planejamento/aprovação de frete** (RBV9-INV-029, §7) — ✅ **CLOSED em produção**: passa pelo plano Campaign antes de materializar fretes; approval por `campaign.approve`, não role hardcoded; imutabilidade do plano aprovado provada por teste. Replan **após** aprovação (execução já em curso) permanece deliberadamente deferido — ver §OPERATION_ORCHESTRATOR_V1.
