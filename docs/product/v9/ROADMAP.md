...
- **E1.3 Separação financeira** (RBV9-INV-055, D-035): **em branch/CI** (`feature/separacao-financeira-operacional-saas`, base `e718eb3`; **NÃO mergeado** — hold do `MAIN_FREEZE=e718eb3` até o deploy do backend P2). Auditoria congelada: backend operacional **já não toca** tabelas SaaS (separado no nível de service); regra "quais fretes contam" **já centralizada** (`agregacaoFinanceiraFretes`, receita realizada = só `finalizado`). **Entregue nesta fatia:** fonte ÚNICA das regras de **status de lançamento** (`STATUS_LANCAMENTO_EFETIVADO` = aprovado/finalizado; `STATUS_LANCAMENTO_NAO_COMPOE` = cancelado/rejeitado) no helper canônico, eliminando literais repetidos em dashboard/relatórios/rentabilidade/acerto (zero mudança de comportamento; testes de equivalência). **Diferido (`DEFERRED ≠ DONE`):** `Dashboard.tsx` operacional ainda embute KPIs **SaaS** super-admin (MRR/inadimplência via "fusão da Visão Geral") — separação é decisão de UX; ver [E1.3_SEPARACAO_FINANCEIRA](./E1.3_SEPARACAO_FINANCEIRA.md).
- **E1.4 Scanner + viewer PDF-first** (RBV9-INV-044/047, D-016): pendente.
- **E1.5 Permissões templates+overrides** (RBV9-INV-018/019/020, D-006/D-008/D-010) — **P2 IMPLEMENTADO (code-only)**: templates por empresa + overrides individuais + visibility policy do motorista; migration **072** (aditiva), resolver único, RPCs guardadas de governança, API `/admin/permissions/*`, web "Perfis e Permissões", app gate por efetivo. `freight.create`/`freight.finish` e financeiro atribuível separados; redação financeira no backend. **`READY_FOR_OWNER_MIGRATION_GATE`** (072 não aplicada; PR aberto). Fundação para dashboards por papel (D-007) e check-in/out (Onda 2).
