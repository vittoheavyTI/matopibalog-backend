...
**Status:** Campaign-B materialization (078), Campaign-C progress/dispatch readiness, **Dispatch V1** (079 — designação/oferta/aceite concorrência-safe) e **Operation Orchestrator V1** (E3.1/E3.2 — objetivo guiado + `next_action`, schema-free) estão **fechados em produção**. Frente concluída em paralelo: `ROUTE_INTELLIGENCE_V1` (provider-agnostic, sem schema/secret/provider real). **E3.5 Portal do Embarcador — PORTAL-A** (fronteira externa + solicitação + handoff, migration 080) também está fechado em produção. **PORTAL-B** (acesso externo, revisão auditável, acompanhamento, documentos/comprovantes) está **implementado e PARADO no gate de migration** — PR #477, migration 081 **não aplicada**, nada de portal em produção ainda. Partner Network, Marketplace, ERP Hub, Fiscal e ações de escrita da IA permanecem `ROADMAP` até novo gate/escopo explícito do owner.
