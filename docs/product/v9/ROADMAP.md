...
- **E3.5 Portal do Embarcador** (RBV9-INV-081, D-024) — 🔶 **PORTAL-A `TECHNICALLY_CLOSED` em produção** (PR #475, `MERGE_SHA=91a52353`; migration `080_shipper_portal_foundation.sql` aplicada uma única vez, 0 linha de negócio). Auditoria confirmou que não existia nenhuma entidade de embarcador (0 tabelas) e que `middlewares/tenant.js` derivaria o tenant interno inteiro se o embarcador fosse criado como `usuarios` — por isso a identidade externa vive em tabelas próprias sem `empresa_id`, e o acesso passa por relacionamento explícito e revogável. **Slice A** (identidade/fronteira) + **Slice B** (solicitação + handoff ao Orchestrator, sem redigitação) = `TECHNICALLY_CLOSED`. **Slice C** (tracking/documentos/comprovantes) e **Slice D** (UX externa do portal) = `DEFERRED_TO_PORTAL_B`, **não iniciados**. Atenção ao ler o estado: PORTAL-A é fundação de domínio — **não há rota de portal em produção** e nenhum embarcador consegue entrar ainda; a única mudança viva em runtime é a auth interna rejeitar token de portal. Ver [SHIPPER_PORTAL_V1](./SHIPPER_PORTAL_V1.md).
