...
### D-081 — A identidade de um evento ERP é a OCORRÊNCIA LÓGICA; o intent é guarda de conflito (ARQ)
`ERP_EVENT_IDENTITY=LOGICAL_EVENT_ID` e `ERP_INTENT_FINGERPRINT=CONFLICT_GUARD`. A chave de idempotência é `provider|empresa_id|event_id|schema_version` — **nunca o payload**. `event_id` passa a significar a identidade estável de uma OCORRÊNCIA: evento novo, `event_id` novo; retry do mesmo evento, mesmo `event_id` (`EVENT_ID_CREATED_ONCE_PER_LOGICAL_EVENT` — `buildEnvelope` gera UUID só ao CRIAR e preserva exatamente o `eventId` fornecido). **Por que a autoridade anterior (`CANONICAL_INTENT_FINGERPRINT`) estava errada**: ela resolvia o colapso de revisões consecutivas mas quebrava em A→B→A — `status=A`, `status=B`, `status=A` novamente; a terceira ocorrência tem payload idêntico à primeira, seria descartada como replay e o ERP ficaria parado em B, em silêncio. Conteúdo repetido não é evento repetido. O fingerprint canônico da INTENÇÃO (`schema_version, entity_type, entity_id, event_type, source, payload`; **sem** `request_id`/`correlation_id`/`occurred_at`/`metadata`) continua existindo com papel estritamente distinto: mesmo `event_id` + mesmo fingerprint = replay idempotente; mesmo `event_id` + fingerprint diferente = **IDEMPOTENCY_CONFLICT**, nunca duplicata benigna. `schema_version` **entra** na chave: mudar a forma do envelope reemite, deliberadamente.

### D-082 — Posse por lease expirável, e recuperação ambígua RECONCILIA antes de reenviar (ARQ)
`ERP_OUTBOX_RECOVERY=LEASE_WITH_RECLAIM` **+** `ERP_OUTBOX_AMBIGUOUS_RECOVERY=RECONCILE_BEFORE_RESEND`. Reivindicar concede `claimToken` e `lease_expires_at`; `markProcessed`/`markFailed`/`recordReconcile` exigem o token vigente, então um worker que volta do limbo não finaliza o trabalho de quem o substituiu (`stale_claim`). O que a R2 ainda errava: lease vencido devolvia o item **para envio**. Um worker pode ter morrido DEPOIS de o ERP aplicar o efeito e antes de confirmar — reenviar ali duplica efeito de negócio real. O trabalho passou a ser **tipado** (`CLAIM_FOR_SEND` vs `CLAIM_FOR_RECONCILE`): lease vencido só libera RECONCILE, qualquer que fosse a ação anterior. O resultado do reconcile está no CONTRATO da máquina: SUCCEEDED→`processed` (zero reenvio); NOT_FOUND→SEND autorizado; PENDING/UNKNOWN→reconciliar de novo (com teto, para não girar para sempre); FAILED→SEND só com `retry_safe:true`, senão fica **bloqueado** para revisão. Contadores separados (`send_attempts` × `reconcile_attempts`): um recovery não consome tentativa de envio. Como corolário, o envelope enfileirado é **snapshot profundo imutável** — mutar a referência do caller, o item devolvido ou o read model não altera a fila; o que foi validado é o que será enviado. O estado honesto continua **`CRASH_SAFE_CONTRACT_DEFINED`**: a semântica de recuperação existe e está provada (incluindo um cenário de crash que conta chamadas reais ao provider), a durabilidade chega com a persistência em E3.7B.

### D-083 — Retry após FAILED exige evidência explícita, e quem aplica isso é a MÁQUINA (SEGURANÇA/ARQ)
`ERP_RETRY_POLICY=NOT_FOUND_ONLY_BY_DEFAULT`, **aplicada pela máquina de outbox**. Provider-agnostic, "falhou" é ambíguo: pode ser "o ERP recusou e nada foi aplicado" ou "o ERP aplicou e a resposta se perdeu". Reenviar no segundo caso duplica efeito real, então `safeToRetry(FAILED)` é `false` por padrão e só `NOT_FOUND` autoriza genericamente; `UNKNOWN` e `PENDING` nunca autorizam. O que a R2 ainda errava: essa política vivia num helper que **o outbox não consultava**. `markFailed` agendava `next_retry_at` e o item voltava elegível para envio quando o backoff vencia — a máquina autorizava, sozinha, exatamente o reenvio proibido. Agora falha de SEND sem evidência **não reabre SEND**: reabre RECONCILE. A única porta para reenvio direto é a evidência explícita (`retrySafe:true` no `markFailed`, ou `evidence.retry_safe===true` no `recordReconcile`) — o ônus da prova fica com quem tem a informação, e o Hub segue sem conhecer fornecedor nenhum.

### D-084 — Diagnóstico de ERP não abre acesso operacional de ERP (SEGURANÇA)
`ERP_DIAGNOSTICS_AUTHORITY=EFFECTIVE_PERMISSION('integracoes_erp.gerenciar')`, com a precedência `ENTITLEMENT → OVERRIDE → TEMPLATE → DEFAULT_DENY` **preservada** e nenhuma permissão nova criada. Como essa chave tem `entitlementCodigo:'integracoes_erp'` e a funcionalidade é `em_breve`, o resolver nega antes de olhar template/override: hoje **nenhum tenant** vê o diagnóstico — nem com override `allow` explícito — e só o super-admin consulta, como autoridade de plataforma. Quando o ERP virar `disponivel` e o plano conceder, a mesma rota passa a responder sem nada precisar ser afrouxado. Corolário de processo: um teste que injeta a permissão direto no efetivo **prova um acesso que ninguém tem** — a autoridade tem de ser exercitada pelo resolver real.
