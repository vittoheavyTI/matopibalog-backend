...
### D-081 — A identidade de um evento ERP é o INTENT canônico, não a entidade (ARQ)
`ERP_IDEMPOTENCY_EVENT_AUTHORITY=CANONICAL_INTENT_FINGERPRINT`. A chave de idempotência é a identidade da entidade (`provider|empresa|entity_type|entity_id|event_type|schema_version`) **mais o fingerprint canônico do `payload`**. Razão: uma chave só de entidade é estável em retry mas **colapsa revisões legítimas** — "frete X updated (valor 100)" e "frete X updated (valor 250)" viravam a mesma chave e a segunda sumia em silêncio. Autoridade **única** e sempre presente; `request_id` foi recusado como autoridade justamente por ser nullable, o que exigiria um fallback e criaria uma segunda autoridade divergente. Excluídos de propósito: `event_id`, `request_id`, `correlation_id`, `occurred_at`, `metadata` — são da tentativa, não do evento. `schema_version` **entra** na chave: mudar a forma do envelope reemite, deliberadamente.

### D-082 — Posse de item de outbox é por lease expirável, e claim obsoleto não finaliza (ARQ)
`ERP_OUTBOX_RECOVERY=LEASE_WITH_RECLAIM`. Reivindicar um item concede um `claimToken` e um `lease_expires_at`. Um item preso em `processing` com lease vencido é **reclamado** deterministicamente (o worker que o segurava provavelmente morreu); o reclaim emite um token novo, e `markProcessed`/`markFailed` exigem o token vigente — assim um worker que volta do limbo não finaliza o trabalho de quem o substituiu (`stale_claim`). Sem isso, um crash travaria o item para sempre e a fila pararia de progredir em silêncio. O estado honesto do contrato é **`CRASH_SAFE_CONTRACT_DEFINED`**, não "production crash-safe": a semântica de recuperação existe e está provada, a durabilidade não — ela chega com a persistência em E3.7B.

### D-083 — Retry após FAILED exige evidência explícita do provider (SEGURANÇA/ARQ)
`ERP_RETRY_POLICY=NOT_FOUND_ONLY_BY_DEFAULT`. Provider-agnostic, "falhou" é ambíguo: pode ser "o ERP recusou e nada foi aplicado" ou "o ERP aplicou e a resposta se perdeu no transporte". Reenviar no segundo caso **duplica um efeito de negócio real**. Como o Hub não pode distinguir, `safeToRetry(FAILED)` é `false` por padrão; só `NOT_FOUND` autoriza reenvio genericamente. Um provider que consiga provar a distinção informa `retry_safe:true` — o ônus da prova fica com quem tem a informação, e o Hub segue sem conhecer fornecedor nenhum. `UNKNOWN` e `PENDING` nunca autorizam, em nenhuma hipótese.
