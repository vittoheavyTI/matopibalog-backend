...
| USR-001 | **Provisionamento de equipe da empresa (`PROVISIONAMENTO_DE_EQUIPE_INCOMPLETO`)** | PARTIAL | ✓ ~ — ✓ ~ | **Lacuna observada em produção pelo owner (2026-08-25).** A arquitetura de permissões existe e está fechada — templates por empresa, overrides, resolver único (`RBV9-INV-016`, migration 072) —, mas a tela de criação de usuário oferece **apenas `Administrador`**. Na prática a empresa não consegue montar a equipe pretendida (gerente de frota, operador) pela UI normal, então o modelo de permissões correto fica inacessível e todo mundo vira admin. Escopo esperado de `TEAM_USER_PROVISIONING_V1`: criar usuário, escolher template/papel, atribuir escopo operacional, overrides individuais, ciclo de vida de status, fluxo de convite/acesso, invariante do último admin, revogação de sessão, isolamento de tenant e auditoria. **Não implementado na fatia de correção visual do Portal** — registrado, não iniciado (§58). Próxima macrofrente. |
