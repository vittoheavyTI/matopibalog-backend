...
| ID | Item | Status | B W A D P | Evidência / Obs |
|----|------|--------|-----------|-----------------|
| RBV9-INV-029 | Planejamento/oportunidade → aprovação → designação | IN_PROGRESS_GATE | ✓ ✓ ✓ ✓ ~ | **Campaign-A implementada no PR #457, mas não CLOSED**. Migration 076 aplicada/rastreada uma vez (`20260823111859`, SHA256 `C7CA4533B9A26B5CCDB04EA9C9913B986432ECC17E8D76D07F302F21C3EFCD94`) criou a fundação em produção, porém pós-check detectou drift count 1 no FK `campaign_exceptions_plan_campaign_fk`. Migration 077 corretiva está criada/certificada e `READY_FOR_OWNER_MIGRATION_GATE_CAMPAIGN_077`; 077 não aplicada, #457 não mergeado/deployado. Sem materialização de fretes e sem mapping comercial. |
...
| PROCESS-004 | `MIGRATION_076_SOURCE_PAYLOAD_PRODUCTION_DRIFT` — migration 076 foi aplicada e rastreada com sucesso em produção, mas o pós-check full catalog encontrou um drift estrutural: `campaign_exceptions_plan_campaign_fk` em produção ficou com `(plan_version_id, empresa_id)` enquanto o HEAD canônico exige `(plan_version_id, campaign_id, empresa_id)`. Não houve linha de negócio Campaign, nem correção manual, nem retry da 076. | READY_FOR_OWNER_CORRECTIVE_GATE | 076 congelada e não deve ser reexecutada/editada. Migration 077 corretiva criada no PR #457 para substituir somente esse FK; CI 077 verde; G0.1 read-only confirmou 077 tracking ausente, FK drift ainda presente, `campaign_exceptions` 0 rows e precondição parent unique OK. Ação corretiva permanente: pós-apply de migration deve incluir assinatura full catalog production-vs-HEAD, não apenas tracking/schema_migrations. |
