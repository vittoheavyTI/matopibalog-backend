...
| RBV9-INV-110 | `ISADMIN_SIGNIFICA_ROLE_E_NAO_USUARIO_INTERNO` | TECH_DEBT | O middleware `isAdmin` exige `role === 'admin'` e guarda **21 pontos de rota** (dashboard, fretes, relatórios, admin…). Por isso todo usuário interno nasce com `usuarios.tipo='admin'` mesmo quando o perfil dele é Operador: gravar `tipo='operador'` criaria alguém que não abre nem o dashboard, com o rótulo legado vencendo a permissão efetiva. `tipo` = classe de conta; `permission_template_id` = autoridade. **Consequência operacional:** enquanto isto não mudar, o ponteiro de template **não pode ficar nulo** — é o que torna a atomicidade da criação obrigatória, e não um refinamento. **Critério de remoção:** trocar `isAdmin` por "usuário interno" e mover a autorização das 21 rotas para permissão efetiva. |
