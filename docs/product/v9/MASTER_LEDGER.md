...
| ID | Item | Status | B W A D P | Evidência / Obs |
|----|------|--------|-----------|-----------------|
| RBV9-INV-029 | Planejamento/oportunidade → aprovação → designação | ROADMAP | ✗ ✗ ✗ ✗ ✗ | **Operation Campaign architecture + owner decisions frozen**: Campaign modelada como objetivo operacional versionado antes de fretes; entitlement tecnico `operation_campaign`; mapping comercial diferido; Campaign-A termina em `APPROVED_PLAN` sem materializar `fretes`; multi-unidade V1 suportada sob scope efetivo completo. Planned trips, scenarios, approval e verifiability definidos em [OPERATION_CAMPAIGN_ARCHITECTURE](./OPERATION_CAMPAIGN_ARCHITECTURE.md). Execucao paralela preparada em [PARALLEL_EXECUTION_BOARD](./PARALLEL_EXECUTION_BOARD.md). Sem implementação, sem migration 076 criada, 0 tabelas Campaign em produção. |
