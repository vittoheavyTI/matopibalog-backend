...
| USR-001 | **Provisionamento de equipe da empresa** | IMPL_NV | ✓ ✓ — ✓ ~ | **Implementado em `TEAM_USER_PROVISIONING_V1` (PR do dia 2026-08-25), schema-free — nenhuma tabela, coluna ou RPC nova.** A lacuna era de três camadas e as três caíram juntas: a tela tinha um campo "Nível" com uma única opção (`admin`); `createUsuario` recusava explicitamente `tipo=operador`; e `GET /permissions/templates` exigia `permissions.manage`, então quem tinha só `users.manage` nem conseguia **ver** os perfis. Agora: `GET /admin/perfis-acesso` + `PUT /admin/usuarios/:id/perfil-acesso` sob `users.manage`, com **contenção de privilégio no servidor** (só se atribui perfil contido no efetivo do ator — conferida na listagem *e* na gravação), motorista/embarcador não atribuíveis pela tela de equipe, criação **atômica** (falha no template desfaz usuário + identidade Auth) e idempotência por `client_request_id`. Invariantes de último admin, revogação de sessão e isolamento de tenant seguem nas RPCs `SECURITY DEFINER` que já existiam. `usuarios.tipo` passa a ser **classe de conta**, não papel (ver `RBV9-INV-110`). Escopo operacional **não** entrou no modal: produção tem 0 grupos/0 unidades/0 memberships. **`IMPL_NV`: sem validação visual do owner.** **Em produção desde 2026-08-25** (PR #481 `MERGE_SHA=a4cc1718`; correção PR #482 `MERGE_SHA=4483c198`). A certificação pós-deploy pegou um defeito da própria frente: o backend passava a devolver `perfil_acesso_nome` e a tela descartava o campo, então a lista chamaria de "Administrador" até quem fosse Operador — corrigido no #482, com dois testes travando a regressão. Certificação read-only confirmou produção inalterada: 18 admin + 20 motorista, 0 usuários criados, 225 templates e 8 overrides intactos, 0 memberships operacionais. Ver [TEAM_USER_PROVISIONING_V1](./TEAM_USER_PROVISIONING_V1.md). |
...
| ID | Item | Status | B W A D P | Evidência / Obs |
|----|------|--------|-----------|-----------------|
| RBV9-INV-081 | Portal do Embarcador (demanda/cotação/proposta) | IMPL_NV | ✓ ✓ — ✓ ~ | D-024. **PORTAL-A `TECHNICALLY_CLOSED` em produção** (PR #475, `MERGE_SHA=91a52353`; migration `080_shipper_portal_foundation.sql` SHA256 `FEF45F5030930C40EEF00B7E83C2A6292D7D5C91597D85BF27ADB546418F7791` **aplicada uma única vez**, tracking `20260825005339`; 0 linha de negócio). Permanece `PARTIAL` porque PORTAL-A entrega **domínio e fronteira**, não superfície HTTP: os serviços não são importados por nenhuma rota, e a única mudança viva em runtime é `middlewares/auth.js` rejeitando `token_kind='shipper_portal'` no sistema interno. Auditoria (§10) achou 0 entidades de embarcador e o risco central: `middlewares/tenant.js` deriva `req.empresa_id` de `usuarios.empresa_id`, então criar o embarcador como usuário interno lhe daria o tenant inteiro. Modelo: identidade externa em tabelas próprias **sem `empresa_id`**; acesso só via `shipper_carrier_relationships` ativo e revogável; isolamento entre embarcadores da MESMA transportadora garantido por FKs compostas no banco (tenant sozinho não basta). Credenciais separadas simetricamente. Solicitação multi-origem com snapshot imutável e unidade única por solicitação (sem conversão kg↔ton); criação+submissão atômica em uma transação; aceite atômico via RPC + handoff idempotente ao Operation Orchestrator — operador não redigita nada. Zero grant a `anon`/`authenticated` (backend-mediado). 26 testes de fronteira + 37 PG reais. Slice C (tracking/docs/comprovantes) e Slice D (UX externa) = **PORTAL-B `TECHNICALLY_CLOSED` em produção** (owner execution 2026-08-25, PR #477 `MERGE_SHA=75a39d0a6758e50cc198a65576a909bbce59446e`; migration `081_shipper_portal_b_revision_documents.sql` SHA256 `285694FBFB4778D38EACDD5D3AC5D3DA75EA80FD462FF1A2737D69DE0711A53E` **aplicada uma única vez** sob `MIGRATION_APPLY_MAX_ATTEMPTS=1`, tracking `20260825144011`). A fidelidade da fonte foi provada no servidor **antes** do apply (digest read-only = SHA certificado) e o SQL registrado no tracking tem o mesmo hash. `PRODUCTION_BUSINESS_DML=0`; só 50 linhas técnicas de permissão (`shipper_portal.documents.share`: 25 administrador + 25 gerente_frota; **operador 0**), backfill histórico 0. PORTAL-B acrescenta: superfície HTTP externa (`/portal/embarcador`) e interna (`/shipper-inbox`) que nunca compartilham contexto; ativação de convite atômica com convergência para falha parcial entre Auth e domínio; revisão auditável append-only (a decisão é carimbada NA versão avaliada, então o aceite sempre diz a qual envio se refere); projeção de acompanhamento que reusa o mapa congelado de status e nunca fabrica progresso; e uma autoridade explícita e revogável de visibilidade de documento, que **não existia** no schema. Aceitar exige `requests.review` E `campaign.create`. Passa a `IMPL_NV` porque tudo isso **está em produção**: `/portal/embarcador/*` e `/shipper-inbox/*` respondem `401` sem credencial (antes `404`), com `/health` 200 e zero erro novo. O que ainda falta é validação **humana**: `OWNER_VISUAL_VALIDATION=PENDING` (login externo, pedido, revisão, acompanhamento, documentos/comprovantes, caixa de entrada). Certificação de produção foi **read-only**: `PRODUCTION_AUTH_USERS_CREATED=0`, `PRODUCTION_INVITATIONS_CREATED=0`, `PRODUCTION_EMAILS_SENT=0`, `PRODUCTION_BUSINESS_WRITES=0` — nenhum embarcador existe ainda. Entrega de convite = `MANUAL_LINK`; **não** há envio de e-mail. Fechar E3.5 **não** cria Partner Network, rede de cotação nem Marketplace. 31 testes de fronteira + 52 PG reais (081) + 37 PG (080) + 27 web. **Correção da aceitação visual aplicada (2026-08-25)**: os 5 HIGH e os 6 MEDIUM do pacote de QA (PR #479) foram fechados — destaque semântico dos cartões (que renderizavam brancos), progresso de entrega visível (`entrega.entregue`/`restante`, que o backend já calculava e a tela ignorava), entrega parcial deixando de sumir da home e da lista, layout do celular, overflow do cabeçalho com nome longo, confirmação de envio, tons de situação, seletor de arquivo em pt-BR, pré-visualização de documento, comparativo de revisão no portal externo e vocabulário externo congelado em **pedido** (navegação Início/Pedidos/Transportes/Documentos). Schema-free. Segue **IMPL_NV**:  até o owner revisar o pacote `portal-v1-owner-visual-after/`. Cotação/proposta **deferida** (`PORTAL_QUOTE_PROPOSAL_V1B`): não há autoridade canônica de preço embarcador↔transportadora. Ver [SHIPPER_PORTAL_V1](./SHIPPER_PORTAL_V1.md). |
| RBV9-INV-082 | **Rede privada de parceiros (E3.6A)** | IMPL_NV | ✓ ✓ — ✓ ~ | D-025/D-026. **`TECHNICALLY_CLOSED` em produção 2026-09-01** — migration `082_partner_network_foundation.sql` **aplicada exatamente uma vez** (tracking `20260901031616`), SHA256 `94787098C9FCDDF6921A38D3F79111E6B877F996FCB04BE3D68043CF6361C183`. A fidelidade foi provada **duas vezes**: o SHA do texto foi conferido no servidor **antes** do apply, e o SHA do SQL gravado no tracking bate byte a byte com o autorizado. Certificação estrutural: 8 tabelas, 34 índices (23 declarados + 8 de PK + 3 de UNIQUE), 11 funções (8 RPCs `SECURITY DEFINER` com `search_path` fixo e EXECUTE só para `service_role`; 3 de trigger), 3 triggers, 6 FKs compostas, RLS 8/8, **0 policies**, **0 grants** a `anon`/`authenticated`. DML técnico exato: `funcionalidades` 23→24 e `permission_template_permissions` 3725→3925 (administrador 100, gerente_frota 100, **operador 0**). `BUSINESS_DML=0` — as 8 tabelas de rede estão **vazias**, e nenhum parceiro, convite, Auth user ou e-mail foi criado. Comercialmente inerte: `plano_funcionalidades`=0 e `empresa_funcionalidades`=0 (`PARTNER_NETWORK_COMMERCIAL_DEFAULT=DENY`). Segue `IMPL_NV` e **não** `IMPL_VAL` porque falta a validação visual do owner: o schema existe e a superfície responde, mas **nenhuma transportadora montou rede ainda** e a entrega de convite continua `MANUAL_LINK` (não há envio de e-mail). A auditoria congelada (E36A_AUDIT_FROZEN=true) encontrou o ponto que mudou a implementação: o campo chamado `capacity_gap_quantity` **não é o residual** — devolve a soma de TODAS as demandas quando existe qualquer exceção HARD_CONSTRAINT, e soma `target_quantity` de unidades diferentes sem converter. Compartilhar isso seria pedir capacidade para um número inventado, então a fonte usada é `campaignProgressService` (residual canônico, unidade preservada, UNKNOWN != ZERO). Modelo: organização parceira é identidade **lógica** (Lite vs Cliente derivado de `linked_empresa_id`, sem coluna paralela que divirja); identidade externa em tabela própria **sem `empresa_id`** — mesma lição da E3.5, porque `middlewares/tenant.js` deriva o tenant de `usuarios.empresa_id`; isolamento por **FK composta** (id, empresa_id) em share, destinatário e resposta, não por checagem de aplicação; snapshot congelado por **trigger** e respostas **append-only** por trigger. O token externo do parceiro é recusado no sistema interno, e a lista de kinds externos virou default-deny com teste que cobra o cadastro de kinds novos. Entitlement `partner_network` nasce DEFERRED_DEFAULT_DENY (nenhum plano, nenhum override); permissões vão para Administrador e Gerente de Frota, **Operador DEFAULT_DENY**. **Sem preço, sem adjudicação, sem marketplace** — E36B_PRICE_AND_AWARD_PRODUCT_DECISION_GATE=PENDING. **Correção da owner review R2 (2026-08-31)**, ainda antes do gate — a 082 foi alterada **no lugar** (sem 083), novo SHA256 `F8C737782334E71EFC975F3BB40C5C1D252B1B5BD5C25EC9C6E8B2B2A17838D8`. Sete achados com um fio comum: a promessa estava escrita e o mecanismo não a sustentava. `HIGH-09` — a ativação fazia `body.email || convite.email`, então o corpo **substituía** o alvo e a prova de senha passava a provar a identidade errada: quem tivesse o link entrava no lugar do convidado. Agora `body.email` só confirma, e um preflight não-consumidor recusa convite morto **antes** de qualquer conta nascer no Auth. `HIGH-10` — o serviço lia dois campos sem o prefixo `out_`, que chegavam `undefined` e viravam claims ausentes no JWT recusado pelo próprio middleware: a ativação inteira estava quebrada e a bateria passava, porque nenhum teste ia da RPC até o token. `HIGH-11` — `UPDATE` + `registrarEvento()` commitavam separado, então uma revogação podia valer sem registro; duas RPCs novas fazem estado e evento na mesma transação, a máquina de estados desceu para o banco e o gravador avulso foi **removido** (mantê-lo manteria a armadilha). `HIGH-12` — `client_request_id` era resolvido antes da autorização e virava chave-mestra de leitura: devolvia 200 depois da revogação, com a organização errada, com a fonte obsoleta ou com o prazo vencido; a ordem agora termina na idempotência, e o ator passou a ser vinculado. `HIGH-13` — a RPC marcava `STALE_SOURCE`, gravava o evento e dava `RAISE`, que desfazia as duas escritas: a auto-correção prometida não existia e a oportunidade seguia `CURRENT` para sempre; agora devolve resultado estruturado e commita. `HIGH-14` — `[A,B,C]` com B revogado compartilhava com A e C e devolvia 201, deixando o operador crer que a lacuna estava coberta; passou a `ALL_REQUESTED_OR_FAIL`, e o teste que consagrava o parcial foi substituído. `HIGH-15` — o login usava `maybeSingle()` em `auth_user_id`, que **falha** com duas linhas, então aceitar o segundo convite quebrava o login do primeiro; sem unicidade global e sem auto-link, passou a `EXPLICIT_CONTEXT_SELECTION`. Resíduos fechados: `approvePlan` lê `{ error }` da RPC (o `try/catch` nunca disparava, porque o client não lança) e o metadata de identidade deixou de marcar conta de parceiro como `portal_embarcador`. **Correção pre-DDL R3 (2026-08-31)**, ainda antes do gate — a 082 alterada **no lugar** de novo, SHA256 final `94787098C9FCDDF6921A38D3F79111E6B877F996FCB04BE3D68043CF6361C183`. Três achados, todos sobre **ordem** e **escopo**. `HIGH-16` — a idempotência do share comparava só campanha e plano, então repetir a chave com outra lista de parceiros devolvia o share antigo como idempotente: `key=ABC` com `[A,B]` e depois com `[A,C]` confirmava um pedido a **C** que nunca existiu. A intenção passou a ser comparada inteira, incluindo o **conjunto normalizado de destinatários** (`[A,B]==[B,A]`, `[A,A,B]==[A,B]`, `[A,B]!=[A,C]`), contra a linha **persistida** — sem coluna nova e sem fingerprint que pudesse divergir dela —, com a normalização feita uma vez só e reusada na comparação e na escrita. `HIGH-17` — a ordem era campanha → idempotência → plano aprovado, então um `client_request_id` antigo ressuscitava um plano superado com `out_idempotent=true`; a autoridade da fonte subiu para antes da resolução da chave (idempotência responde "já foi feita?", nunca antes de "ainda é válida?"). `HIGH-18` — `mark_source_stale` marcava toda oportunidade `CURRENT` da campanha sem olhar a fonte, e o `approvePlan` a chama **depois** de promover o plano novo: um share do plano NOVO era marcado obsoleto pelo próprio replan que o aprovou, e o parceiro via como vencido o único pedido válido. Agora o marcador lê `approved_plan_version_id` sob lock e só marca `plan_version_id IS DISTINCT FROM` essa fonte (`IS DISTINCT FROM` porque a autoridade pode ser NULL). **Uma autoridade só** — o status do plano segue conferido onde decide de fato, na transação de `submit_response`, que não depende desta marcação (as duas camadas são complementares e a segunda foi preservada). **111 PG reais** (corrida de duas conexões para share × revoke **e** para share-antigo-em-voo × replan; rollback de auditoria; marcador idempotente) + **2017 backend** + **230 web** + SEC-1; 10/10 CI verdes. `PRODUCTION_DDL=0`, `PRODUCTION_BUSINESS_WRITES=0`. Ver [PARTNER_NETWORK_V1](./PARTNER_NETWORK_V1.md). |
...
| UX-FORM-001 | **Padrão de formulário do painel (`FREIGHT_MODAL_PATTERN_V1`)** | IMPL_NV | — ✓ — — ~ | Congelado a partir do modal de **Novo Frete**, que o owner já aprovou: overlay `fixed inset-0`, container `rounded-2xl max-h-[90vh] flex flex-col`, cabeçalho e rodapé fixos, **corpo com rolagem própria**, rodapé com `Cancelar` + ação primária sempre visível. Extraído para `components/ModalFormulario.tsx` e adotado por **Novo Usuário** e **Novo Motorista**. O Frete **não** foi refatorado (decisão, não omissão: o modal vive num arquivo de 2.000+ linhas entrelaçado com cálculo de valores — ver `RBV9-INV-111`). Pacote visual com antes/depois em `team-user-provisioning-visual/`. |
| TEAM-UX-001 | **Chave técnica de permissão não é informação de usuário** | IMPL_NV | ✓ ✓ — — ~ | `TEAM_UX_001=REMOVE_RAW_PERMISSION_KEYS_FROM_NORMAL_LIST`. A lista de equipe exibia `DASHBOARD`, `MOTORISTAS`, `RELATORIOS`, `CONFIGURACOES` em caixa alta — chaves do campo legado `usuarios.permissoes`, que desde a migration 072 **nem descreve mais o acesso real** (o efetivo vem de template + overrides). Era dado obsoleto com aparência de autoridade. A coluna foi **removida sem substituição**: a informação de acesso da lista é o **Perfil de acesso**, e a tabela caiu de 6 para 5 colunas (ajuda o layout estreito, sem rolagem horizontal nova). Exceção individual aparece como indicador humano — "2 ajustes de acesso" — junto ao perfil, porque **qualifica** o perfil em vez de ser dado independente; o backend passou a devolver `ajustes_de_acesso` numa consulta agregada casada por `(usuário, empresa)`. Nenhuma chave é exibida: o detalhe vive em Perfis e Permissões, sob `permissions.manage`, e o modal aponta para lá — **sem criar um segundo editor**. **Em produção** no bundle `index-8Hc3KmQ2.js`: a coluna `Permissões` não aparece mais e `ajustes de acesso` aparece. Ver [RBV9_INV_110_AUTHORIZATION_CLOSURE](./RBV9_INV_110_AUTHORIZATION_CLOSURE.md). |
| TEAM-VIS/FUNC | **Correção da aceitação visual do owner (modal de usuário)** | IMPL_NV | ✓ ✓ — ✓ ~ | `TEAM_VISUAL_EDIT_FINDINGS_FROZEN=true`. Seis achados visuais e quatro funcionais. O grave era **`TEAM-FUNC-02`**: um Administrador legítimo via apenas "Financeiro" na lista de perfis — e zero num plano menor. A causa não era permissão: `contido()` comparava as permissões **cruas** do template alvo contra o **efetivo** do ator, que já passou pelo gate de entitlement. Como o plano da Empresa Alfa não inclui `operation_campaign` nem `estrutura_operacional`, o efetivo do admin não tem `campaign.*` mas o template tem — então **ele não conseguia atribuir nem o próprio perfil**. Efeito invertido: quanto menor o plano, menos time a empresa montava. Corrigido comparando efetivo com efetivo, nos dois pontos de decisão. **`TEAM-FUNC-01`**: trocar perfil de usuário existente não tinha caminho na UI (o campo era read-only e apontava para a tela errada); agora usa o endpoint canônico, com o `409` de último administrador junto do campo. **`TEAM-FUNC-03`**: o CEP tinha 3 comportamentos em 4 formulários e um bug de estado obsoleto que apagava o CEP digitado quando a busca voltava — resolvido por componente único com patch + updater funcional. Conta virou typeahead, perfil virou seleção recolhida, as seções perderam o gate de Mostrar/Ocultar (prop removida do componente), e o super-admin ganhou "Editar usuário" no modo leitura. E-mail e conta vinculada seguem imutáveis, com razão registrada. **Em produção 2026-08-26**: PR #486 (`5f4708f0`), 4/4 CI verdes com SEC-1 sem rerun, bundle `index-iCm7Pmll.js`; certificação read-only com contrato `401` e banco inalterado (38 usuários, 0 criados, 225 templates, 8 overrides). `TEAM_OWNER_VISUAL_VALIDATION=PENDING_OWNER_FINAL_REVIEW`. Ver [TEAM_OWNER_VISUAL_CORRECTION](./TEAM_OWNER_VISUAL_CORRECTION.md). |
...
| RBV9-INV-110 | `AUTORIDADE_LEGADA_ISADMIN` | **CLOSED** | `AUTHORIZATION_DEBT=CLOSED`. Auditoria congelada e fechada no delta de 2026-08-25 (`RBV9_INV_110_AUDIT_FROZEN=true`). `isAdmin` guardava **80 rotas em 11 pontos de código**, e desde a D-069 não distinguia ninguém interno — todo usuário carrega `tipo=admin`. Classificação: **0 invariantes reais de plataforma**, 64 rotas que eram na verdade super-admin, 14 redundantes e **6 superfícies com `WRONG_ALLOW`**. A mais grave: `PATCH /despesas|abastecimentos|vales/:id` era guardado por `launch.create` e delegava a transição para um gate `role===admin`, então um **Operador aprovava lançamento sem ter `launch.approve`**, contornando a rota dedicada. Também caíram: solicitar upgrade de plano (cria cobrança), atos contratuais, estado comercial da empresa, relatório de aceites (expunha IP/user-agent por usuário) e validação de comprovação de entrega. **Nenhuma permissão nova** — tudo reusa o registry. `isSuperAdmin` e `permissions.manage` intocados; formato do token intocado. **Em produção desde 2026-08-26**: PR #484 `MERGE_SHA=a79a7c66`, 4/4 CI verdes (SEC-1 sem rerun), Railway `72698a05`, bundle `index-8Hc3KmQ2.js`. Certificação read-only: contrato `401` em todas as superfícies tocadas; banco inalterado (18 admin + 20 motorista, 0 usuários criados, 225 templates, 8 overrides, 38/38 com ponteiro de template). Ver [RBV9_INV_110_AUTHORIZATION_CLOSURE](./RBV9_INV_110_AUTHORIZATION_CLOSURE.md). |
| RBV9-INV-112 | `LEGACY_ACCOUNT_CLASS_CLEANUP` | TECH_DEBT | `role==='admin'` permanece em usos que **não são autorização**: escolher o `motorista_id` efetivo do lançamento, definir o status inicial (painel entra aprovado, app entra pendente), selecionar qual regra de *ownership* aplicar, `billingProfile`, `trialV2Service` e filtros de escopo em `fretesController`. É discriminação painel-vs-app, não autoridade. **Não bloqueante porque nenhuma autorização sensível depende mais dela** (RBV9-INV-110 fechado). **Critério de remoção:** um discriminador explícito de origem/classe de conta que não se pareça com papel. Enquanto isso, não reintroduzir `role` como gate. |
| RBV9-INV-111 | `CONVERGENCIA_DOS_TRES_MODAIS` | TECH_DEBT | Usuário e Motorista usam `ModalFormulario`; o Frete reproduz o mesmo comportamento com marcação própria. Extrair o modal de Frete significaria mexer no arquivo que calcula valor de frete e comissão — risco desproporcional ao ganho de uma tela de cadastro. Limpeza técnica, não dívida de produto. |
