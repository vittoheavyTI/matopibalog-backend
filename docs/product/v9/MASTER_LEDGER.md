...
| RBV9-INV-031 | Dispatch estilo Uber (oferta/aceite/expira, lock concorrência-safe) | ROADMAP | ✗ ✗ ✗ ✗ ✗ | D-033 preservada; Campaign prepara necessidade/viagens, Dispatch decide executor. **Campaign-C entrega só a prontidão** (`dispatch readiness`: `READY_FOR_DIRECT_ASSIGNMENT`/`READY_FOR_OFFER_DISPATCH`/`BLOCKED`/`ALREADY_ASSIGNED`/`ALREADY_EXECUTING`/`COMPLETED`), nunca a designação/oferta em si — `REAL_DISPATCH_IMPLEMENTED=false`, `OFFER_SYSTEM_IMPLEMENTED=false`. Oferta para elegíveis e first valid acceptance atômico permanecem ROADMAP. |
...
> **Recalculado após Campaign-C progress + dispatch readiness** (PR #469). Inventário: 109 (`RBV9-INV`) + 20 (`FISC`) = **129**.

- **Total de itens:** **129** (109 RBV9-INV + 20 FISC)
- **IMPLEMENTED_VALIDATED:** 47 (RBV9-INV-032 elegibilidade sai de ROADMAP_ONLY nesta rodada)
- **IMPLEMENTED_NOT_VISUAL_VALIDATED:** 16 (inclui Fleet technical closure aguardando owner visual validation)
- **PARTIAL:** 13
- **ROADMAP_ONLY:** **41** (RBV9-INV-031 dispatch real permanece ROADMAP — Campaign-C entrega só readiness)
