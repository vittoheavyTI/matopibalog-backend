  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (connectivity != ConnectivityResult.none) {
        final success = await ApiService.createMovementWithPhoto('abastecimentos', fields, _image!.path);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            TextField(controller: _postoCtrl, decoration: const InputDecoration(labelText: 'Posto', border: OutlineInputBorder())),
            const SizedBox(height: 16),
            TextField(controller: _litrosCtrl, decoration: const InputDecoration(labelText: 'Litros', border: OutlineInputBorder()), keyboardType: TextInputType.number),
            const SizedBox(height: 16),
            TextField(controller: _valorTotalCtrl, decoration: const InputDecoration(labelText: 'Valor Total (Diesel)', border: OutlineInputBorder()), keyboardType: TextInputType.number),
            const SizedBox(height: 16),
            TextField(controller: _arlaLitrosCtrl, decoration: const InputDecoration(labelText: 'Litros Arla (Opcional)', border: OutlineInputBorder()), keyboardType: TextInputType.number),
            const SizedBox(height: 16),
            TextField(controller: _arlaValorCtrl, decoration: const InputDecoration(labelText: 'Valor Arla (Opcional)', border: OutlineInputBorder()), keyboardType: TextInputType.number),
            const SizedBox(height: 16),
