...
// expandido → lazy), trata vazio/carregando/erro e expõe as ações conforme o
// papel (o motorista registra/anexa; só admin valida ePOD e muda status de
// ocorrência). Reaproveita os endpoints /fretes/:id/epod e /fretes/:id/ocorrencias.

type Epod = {
  id: string; status: 'registrado' | 'validado' | 'rejeitado';
...
type Evidencia = { id: string; nome_arquivo: string | null; mime: string | null; created_at: string };
type Ocorrencia = {
...
function BadgeEpod({ status }: { status: Epod['status'] }) {
  const map = {
    registrado: { cls: 'bg-amber-100 text-amber-700', icon: Clock, txt: 'Registrado' },
  ...
  return (
    <div className="border-t border-gray-100 pt-2 mt-1 space-y-3">
      <BlocoEpod
        ...
        onMudou={carregar} abrirEvidencia={abrirEvidencia}
      />
      <BlocoOcorrencias
        freteId={freteId} ocorrencias={ocorrencias} ehAdmin={ehAdmin} onMudou={carregar}
      />
    </div>
  );
};
...
const BlocoEpod: React.FC<{
  ...
  const registrar = async () => {
    ...
    try {
      await api.post(`/fretes/${freteId}/epod`, {
  ...
  const validar = async (status: 'validado' | 'rejeitado') => {
    setSalvando(true); setErroAcao('');
    try {
      await api.post(`/fretes/${freteId}/epod/validacao`, {
        status, motivo_rejeicao: status === 'rejeitado' ? motivoRejeicao : undefined,
      });
      setRejeitando(false); setMotivoRejeicao('');
      onMudou();
    } catch (e: any) {
      setErroAcao(e?.response?.data?.message || 'Não foi possível validar a comprovação.');
    } finally { setSalvando(false); }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <SecTitle>Comprovação de entrega</SecTitle>
      ...
      {!epod ? (
        ...
        <div className="bg-gray-50 rounded-lg p-2 space-y-1 text-xs text-gray-700">
          ...
          {/* Evidências */}
          <div className="pt-1">
            <p className="text-[11px] font-semibold text-gray-500">Evidências</p>
            {evidencias.length === 0 ? (
              <p className="text-xs text-gray-400">Nenhuma evidência anexada.</p>
            ) : (
              <ul className="space-y-1 mt-1">
                {evidencias.map(ev => (
                  <li key={ev.id} className="flex items-center justify-between gap-2 bg-white rounded px-2 py-1">
                    <span className="truncate text-gray-700">{ev.nome_arquivo || 'Evidência'}</span>
          ...
          {/* Ações do admin: validar / rejeitar */}
          {ehAdmin && epod.status === 'registrado' && (
            <div className="pt-1 border-t border-gray-200 mt-1">
              {!rejeitando ? (
                <div className="flex items-center gap-2">
                  <button disabled={salvando} onClick={() => validar('validado')} className="text-xs bg-green-700 hover:bg-green-800 disabled:opacity-60 text-white rounded px-3 py-1 font-semibold">Validar</button>
                  <button disabled={salvando} onClick={() => setRejeitando(true)} className="text-xs bg-red-50 hover:bg-red-100 text-red-700 rounded px-3 py-1 font-semibold">Rejeitar</button>
                </div>
              ) : (
                <div className="space-y-2">
                  <input value={motivoRejeicao} onChange={e => setMotivoRejeicao(e.target.value)} placeholder="Motivo da rejeição" className="w-full text-xs border border-gray-200 rounded px-2 py-1" />
                  <div className="flex items-center gap-2">
                    <button disabled={salvando || !motivoRejeicao.trim()} onClick={() => validar('rejeitado')} className="text-xs bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white rounded px-3 py-1 font-semibold">Confirmar rejeição</button>
                    <button onClick={() => { setRejeitando(false); setMotivoRejeicao(''); }} className="text-xs text-gray-500 hover:underline">Cancelar</button>
                  </div>
                </div>
              )}
            </div>
