...
export const Contratacao: React.FC = () => {
  ...
  const concluido = !!contrato && ['plenamente_assinado', 'assinado', 'aceito_manualmente'].includes(contrato.status);
  ...
  return (
    <div className="space-y-3 animate-fade-in">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Contratação</h1>
          <p className="text-xs text-gray-500">Contrato emitido pela Matopiba Log e assinado eletronicamente por você, com confirmação por e-mail e registro técnico.</p>
        </div>
        <button
          type="button"
          onClick={carregar}
          disabled={loading}
          title="Atualizar"
          className="inline-flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 shrink-0"
        >
      ...
      {loading ? (
        <div className="bg-white border border-gray-100 rounded-lg p-8 text-sm text-gray-500">Carregando contratação...</div>
      ) : !proposta ? (
        <div className="bg-white border border-gray-100 rounded-lg p-6 text-sm text-gray-600">
          Ainda não há proposta comercial vinculada à sua empresa.
        </div>
      ) : concluido ? (
        <div className="bg-white border border-green-200 rounded-lg p-6 shadow-sm">
          ...
          >
            <FileText size={16} /> Ver plano e contratos
          </Link>
        </div>
      ) : (
        <>
          {/* AÇÃO PRINCIPAL NO TOPO: a assinatura do cliente é a única etapa
              manual. O contrato já nasce emitido/pré-assinado pela Matopiba Log,
              então esta assinatura conclui e libera o sistema automaticamente. */}
          {podeAssinar && (
            <section className="bg-white border-2 border-blue-200 rounded-lg p-4 shadow-sm space-y-3">
              ...
              <div className="grid sm:grid-cols-[1fr_auto] gap-2">
                ...
                >
                  <MailCheck size={16} /> Enviar código
                </button>
              </div>
              {desafio && (
                <div className="space-y-3 rounded-lg bg-blue-50/60 border border-blue-100 p-3">
                  <div className="text-xs text-gray-600">
                    Código enviado para {desafio.email_mascarado || 'seu e-mail'}. Informe o código de 6 dígitos e confirme abaixo.
                  </div>
                  <input
                    type="text"
                    inputMode="numeric"
                    ...
                    className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm tracking-widest outline-none focus:border-blue-500"
                  />
                  <label className="flex items-start gap-2 text-sm text-gray-700">
                    <input type="checkbox" checked={consentimento} onChange={(e) => setConsentimento(e.target.checked)} className="mt-1" />
                    <span>Li o contrato e aceito assiná-lo eletronicamente com código de confirmação.</span>
                  </label>
                  <label className="flex items-start gap-2 text-sm text-gray-700">
                    <input type="checkbox" checked={poderes} onChange={(e) => setPoderes(e.target.checked)} className="mt-1" />
                    <span>Declaro que tenho poderes para representar a empresa nesta contratação.</span>
                  </label>
                  <button
                    type="button"
          ...
          {/* Contingência (fluxo antigo/manual): sua parte assinada, conclusão da Matopiba em processamento. */}
          {contrato?.status === 'aguardando_assinatura_matopiba' && (
            <Aviso tipo="sucesso" texto="Sua assinatura foi registrada. A conclusão institucional da Matopiba está sendo processada." />
          )}

          <div className="grid xl:grid-cols-3 gap-3">
            <section className="xl:col-span-2 bg-white border border-gray-100 rounded-lg p-4 shadow-sm space-y-3">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-base font-semibold text-gray-900">{resumo.plano_nome || 'Plano contratado'}</h2>
                  <p className="text-xs text-gray-500">{statusLabel[proposta.status] || proposta.status}</p>
                </div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">
                ...
                <Info label="Período de teste" value={`${resumo.trial_dias ?? proposta.trial_dias ?? 0} dias`} />
                <Info label="Recorrência" value="Mensal" />
                <Info label="Método" value={contrato?.metodo_assinatura === 'interno_otp' ? 'Código por e-mail' : 'Manual'} />
              </div>
